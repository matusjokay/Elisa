...
class ModelFormOptions(object):
	def __init__(self, options=None):
		...
		self.legends = getattr(options, 'legends', {})
...
class ModelForm(forms.ModelForm):
	...
	def __init__(self, *args, **kwargs):
		...
		if hasattr(self._forms, 'inlines'):
			self.inlineformsets = OrderedDict()
			for key, FormSet in self._forms.inlines.items():
				self.inlineformsets[key] = FormSet(self.data or None, self.files or None, prefix=self._get_formset_prefix(key), instance=self.instance)
		if hasattr(self._forms, 'legends'):
			self.legends = self._forms.legends

	def save(self, *args, **kwargs):
		instance = super(ModelForm, self).save(*args, **kwargs)
		if hasattr(self._forms, 'inlines'):
			for key, FormSet in self._forms.inlines.items():
				fset = FormSet(self.data, self.files or None, prefix=self._get_formset_prefix(key), instance=instance)
				if not hasattr(fset, 'cleaned_data'):
					fset.clean()
				for form in fset.deleted_forms:
					form.instance.delete()
				forms = fset.ordered_forms if hasattr(fset, 'ordered_forms') else fset.forms
				for i, form in enumerate(forms):
					if 'ORDER' in form.cleaned_data.keys():
						form.instance.order = i+1
					# if form in fset.deleted_forms:
					# 	form.instance.delete()
					if form.has_changed() and form.is_valid() and form not in fset.deleted_forms:
						form.save()
		return instance
	...
	def _get_formset_prefix(self, key):
		return u'%s_%s' % (self.prefix, key.lower())
	...
	def _clean_form(self):
		...
		for key, fset in self.inlineformsets.items():
			for i in range(0, fset.total_form_count()):
				form = fset.forms[i]
				if form.errors:
					self._errors['_%s_%d' %(fset.prefix, i)] = form.non_field_errors
